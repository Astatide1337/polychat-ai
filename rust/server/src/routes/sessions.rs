...
pub async fn push_session_handler(
    ...
    let has_cookies = session.get("cookies").and_then(|c| c.as_array()).map_or(false, |a| !a.is_empty());
    let has_origins = session.get("origins").and_then(|o| o.as_array()).map_or(false, |a| !a.is_empty());
    let has_user_token = session.get("userToken").and_then(|v| v.as_str()).map_or(false, |s| !s.is_empty());
